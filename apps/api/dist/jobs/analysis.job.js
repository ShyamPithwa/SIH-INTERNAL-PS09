...
exports.runAnalysisJob = runAnalysisJob;
...
const engine_service_1 = require("../services/engine.service");
...
async function runAnalysisJob(bessId, currentTelemetry) {
    ...
    try {
        ...
        const { data: dbAsset, error: assetErr } = await supabase_1.supabase
            .from('bess_assets')
            .select()
            .eq('id', bessId)
            .single();
        ...
        const filteredHistory = historyTelemetry.filter(h => h.recordedAt !== currentTelemetry.recordedAt);
        ...
        if (result.forecasts) {
            for (const targetName of Object.keys(result.forecasts)) {
                ...
                const points = fc.values.map((val, stepIdx) => {
                    const predictedAt = new Date(new Date(currentTelemetry.recordedAt).getTime() + (stepIdx + 1) * 5 * 60 * 1000).toISOString();
...
exports.default = runAnalysisJob;
